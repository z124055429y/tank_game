...
constexpr int PUSH_SCENE_RESTORE = 1 << 2;
constexpr int PUSH_SCENE_BUILD = 1 << 3;

constexpr int MASK_POP_SCENE    = 0xff00;
constexpr int POP_SCENE_START   = 1 << (0 + 8);
constexpr int POP_SCENE_GAME    = 1 << (1 + 8);
constexpr int POP_SCENE_RESTORE = 1 << (2 + 8);
constexpr int POP_SCENE_BUILD   = 1 << (3 + 8);
...
public:
    virtual ~Scene() {};
    virtual void init() = 0;
    virtual int input(int ch, int x = 0, int y = 0) = 0;
